...
####In this file, 
####mediator intercept and slope effects on IY SY are both Zero (0)
####X effects on  IY SY are both Non-Zero (d=0.5)
...
OneSimu <- function(seed=12,
  ...
  if( rho12_vM1==0 ){
    if (f2IMSM[1]==0 & f2IMSM[2]==0){
      ...
      #if f2IMSM[1]==0, bIMSM[1]=0 automatically (since term f2IMSM[1]*)
      ...
      bZ[2]=-bZ[1]/TimeY[1]
      ##f2bIM= c(0,0);
      bIM[1]=sqrt(f2bIM[1]*vIY/((0.15+1)*Psi_vMctrl[1,1]))*0.9 ##diag(Psi_vMctrl)=diag(Psi_vMTrt)
      # bIM[2]= sqrt(0.15*vSY/((0.15+1)*Psi_vMctrl[1,1]))*0.9
      bIM[2]=-bIM[1]/TimeY[1]
      ##f2bSM= c(0,0); 
      bSM[1]=sqrt(f2bSM[1]*vIY/((0.15+1)*Psi_vMctrl[2,2]))*0.9 ##diag(Psi_vMctrl)=diag(Psi_vMTrt)
    ...
    if (f2IMSM[1]==0 & f2IMSM[2]==0.15){
      ...
      IM=a0[1]+aX[1]*X+aZ[1]*Z + vMtrt[ ,1]*X + vMctrl[ ,1]*(1-X)
      SM=a0[2]+aX[2]*X+aZ[2]*Z + vMtrt[ ,2]*X + vMctrl[ ,2]*(1-X)
      # a0[1]= -(Psi_vMtrt[1,2]-Psi_vMctrl[1,2]+aX[1]*aX[2])/aX[2]
      
      bZ=bIM=bSM=bX=bIMSM=rep(NA, 2)
      bIMSM[1]=sqrt( f2IMSM[1]*vIY/((0.15+1)*Psi_vMctrl[1,1]*Psi_vMctrl[2,2]))*0.75
      bIMSM[2]= sqrt( f2IMSM[2]*vSY/((0.15+1)*Psi_vMctrl[1,1]*Psi_vMctrl[2,2]))*1.2
  ...
  if( rho12_vM1==0.3 ){
    ...
    if (f2IMSM[1]==0.15 & f2IMSM[2]==0.15){
      bZ=bIM=bSM=bX=bIMSM=rep(NA, 2)
      bIMSM[1]=sqrt( f2IMSM[1]*vIY/((0.15+1)*Psi_vMctrl[1,1]*Psi_vMctrl[2,2]))*0.9
      bIMSM[2]=sqrt( f2IMSM[2]*vSY/((0.15+1)*Psi_vMctrl[1,1]*Psi_vMctrl[2,2]))*0.9
      bZ[1]=sqrt(0.02*vIY/((0.15+1)*1))*0.5
      # bZ[2]= sqrt(0.02*vSY/((0.15+1)*1))*0.8
      bZ[2]=-bZ[1]/TimeY[1]
      ##f2bIM= c(0,0);
    ...
    if (f2IMSM[1]==0 & f2IMSM[2]==0.15){
      ...
      bZ=bIM=bSM=bX=bIMSM=rep(NA, 2)
      bIMSM[1]=sqrt( f2IMSM[1]*vIY/((0.15+1)*Psi_vMctrl[1,1]*Psi_vMctrl[2,2]))*0.9
      bIMSM[2]=sqrt( f2IMSM[2]*vSY/((0.15+1)*Psi_vMctrl[1,1]*Psi_vMctrl[2,2]))*1.1
      bZ[1]=sqrt(0.02*vIY/((0.15+1)*1))*1
      # bZ[2]= sqrt(0.02*vSY/((0.15+1)*1))*0.8
      bZ[2]=-bZ[1]/TimeY[1]
      ##f2bIM= c(0,0);
      bIM[1]=sqrt(f2bIM[1]*vIY/((0.15+1)*Psi_vMctrl[1,1]))*0.9 ##diag(Psi_vMctrl)=diag(Psi_vMTrt)
    ...
    if (f2IMSM[1]==0.15 & f2IMSM[2]==0){
      ...
      bZ=bIM=bSM=bX=bIMSM=rep(NA, 2)
      bIMSM[1]=sqrt( f2IMSM[1]*vIY/((0.15+1)*Psi_vMctrl[1,1]*Psi_vMctrl[2,2]))*1
      bIMSM[2]=sqrt( f2IMSM[2]*vSY/((0.15+1)*Psi_vMctrl[1,1]*Psi_vMctrl[2,2]))*0.95
      bZ[1]=sqrt(0.02*vIY/((0.15+1)*1))*1
      # bZ[2]= sqrt(0.02*vSY/((0.15+1)*1))*0.8
      bZ[2]=-bZ[1]/TimeY[1]
      ##f2bIM= c(0,0);
