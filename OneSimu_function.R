...
OneSimu <- function(seed=12,
  ...
  # if(TT==12){
  if( rho12_vM1==0 ){
    if (f2IMSM[1]==0 & f2IMSM[2]==0){
      # 
      bZ=bIM=bSM=bX=bIMSM=rep(NA, 2)
      bIMSM[1]=sqrt( f2IMSM[1]*vIY/((0.15+1)*Psi_vMctrl[1,1]*Psi_vMctrl[2,2]))*0.3
      bIMSM[2]=sqrt( f2IMSM[2]*vSY/((0.15+1)*Psi_vMctrl[1,1]*Psi_vMctrl[2,2]))*0.2
      bZ[1]=sqrt(0.02*vIY/((0.15+1)*1))*0.5
      # bZ[2]= sqrt(0.02*vSY/((0.15+1)*1))*0.8
      bZ[2]=-bZ[1]/TimeY[1]
      bIM[1]=sqrt(0.15*vIY/((0.15+1)*Psi_vMctrl[1,1]))*0.9 ##diag(Psi_vMctrl)=diag(Psi_vMTrt)
      # bIM[2]= sqrt(0.15*vSY/((0.15+1)*Psi_vMctrl[1,1]))*0.9
      bIM[2]=-bIM[1]/TimeY[1]
      bSM[1]=sqrt(0.15*vIY/((0.15+1)*Psi_vMctrl[2,2]))*0.9 ##diag(Psi_vMctrl)=diag(Psi_vMTrt)
      # bSM[2]= sqrt(0.15*vSY/((0.15+1)*Psi_vMctrl[2,2]))*0.9
      bSM[2]=-bSM[1]/TimeY[1]
      bX[1]=0.5*sqrt(vIY)*0.8
      bX[2]= -bX[1]/TimeY[1]
      # bX[1] = -TimeY[1]*(bX[2]+bIM[2]*aX[1]+bSM[2]*aX[2]+bIMSM[2]*(a0[1]*aX[2]+a0[2]*aX[1]+aX[1]*aX[2]+Psi_vMtrt[1,2]-Psi_vMctrl[1,2])) +
      #   (-1)*(bIM[1]*aX[1]+bSM[1]*aX[2]+bIMSM[1]*(a0[1]*aX[2]+a0[2]*aX[1]+aX[2]*aX[2]+Psi_vMtrt[1,2]-Psi_vMctrl[1,2])) 
      # bX[1]= (-1)*(bIM[1]*aX[1]+bSM[1]*aX[2]+bIMSM[1]*(a0[1]*aX[2]+a0[2]*aX[1]+aX[1]*aX[2]+Psi_vMtrt[1,2]-Psi_vMctrl[1,2])) 
      # bX[2]= -(1/TimeY[1])*(bX[1]+ (bIM[1]*aX[1]+bSM[1]*aX[2]+bIMSM[1]*(a0[1]*aX[2]+a0[2]*aX[1]+aX[1]*aX[2]+Psi_vMtrt[1,2]-Psi_vMctrl[1,2])) ) - 
      #   (bIM[2]*aX[1]+bSM[2]*aX[2]+bIMSM[2]*(a0[1]*aX[2]+a0[2]*aX[1]+aX[2]*aX[2]+Psi_vMtrt[1,2]-Psi_vMctrl[1,2])) 
      # bX[2]= (-1)*(bIM[2]*aX[1]+bSM[2]*aX[2]+bIMSM[2]*(a0[1]*aX[2]+a0[2]*aX[1]+aX[1]*aX[2]+Psi_vMtrt[1,2]-Psi_vMctrl[1,2])) 
      
      #check randomization constraint
      # (bX[1] + bIM[1]*aX[1]+bSM[1]*aX[2]+bIMSM[1]*(a0[1]*aX[2]+a0[2]*aX[1]+aX[1]*aX[2]+Psi_vMtrt[1,2]-Psi_vMctrl[1,2])) +TimeY[1]*(bX[2] + bIM[2]*aX[1]+bSM[2]*aX[2]+bIMSM[2]*(a0[1]*aX[2]+a0[2]*aX[1]+aX[1]*aX[2]+Psi_vMtrt[1,2]-Psi_vMctrl[1,2])) 
      ...
      # TT=12: 5.5; TT=6: 5.3; TT=3: 5.3
      Psi_vY=diag(c(NA, 5.4),2,2)
      Psi_vY[1,1]=(TimeY[1]^2)*Psi_vY[2,2]
    }
    
    if (f2IMSM[1]==0.15 & f2IMSM[2]==0.15){
      bZ=bIM=bSM=bX=bIMSM=rep(NA, 2)
      bIMSM[1]=sqrt( f2IMSM[1]*vIY/((0.15+1)*Psi_vMctrl[1,1]*Psi_vMctrl[2,2]))*0.75
      bIMSM[2]= -bIMSM[1]/TimeY[1]
      bZ[1]=sqrt(0.02*vIY/((0.15+1)*1))*0.1
      # bZ[2]= sqrt(0.02*vSY/((0.15+1)*1))*0.8
      bZ[2]=-bZ[1]/TimeY[1]
      bIM[1]=sqrt(0.15*vIY/((0.15+1)*Psi_vMctrl[1,1]))*0.8 ##diag(Psi_vMctrl)=diag(Psi_vMTrt)
      # bIM[2]= sqrt(0.15*vSY/((0.15+1)*Psi_vMctrl[1,1]))*0.9
      bIM[2]=-bIM[1]/TimeY[1]
      bSM[1]=sqrt(0.15*vIY/((0.15+1)*Psi_vMctrl[2,2]))*0.8 ##diag(Psi_vMctrl)=diag(Psi_vMTrt)
      # bSM[2]= sqrt(0.15*vSY/((0.15+1)*Psi_vMctrl[2,2]))*0.9
      bSM[2]=-bSM[1]/TimeY[1]
      bX[1]=0.5*sqrt(vIY)*0.3
      bX[2]= -bX[1]/TimeY[1]
      ...
      # TT=12: 4.8; TT=6: 5.3; TT=3: 5.3
      Psi_vY=diag(c(NA, 5.4),2,2)
      Psi_vY[1,1]=(TimeY[1]^2)*Psi_vY[2,2]
    }
    
    # f2IMSM=c(0,0.15)
    if (f2IMSM[1]==0 & f2IMSM[2]==0.15){
      # try seting a0[1:2] to satisfy the randomization constraint on Y1; ## this same following term is multiplied to both bIY.IMSM and bSY.IMSM:  (\alpha_{IM0}\alpha_{SM.X}$+$\alpha_{SM0}\alpha_{IM.X} +  \alpha_{IM.X}\alpha_{SM.X}  + \psi_{v_{IM}v_{SM}}^{Trt}-\psi_{v_{IM}v_{SM}}^{Ctrl})  
      ##set aIM0= -aSM0*TimeM[1] ; parallel to  \alpha_{IM.X}+\alpha_{SM.X}Time_{M1}=0 
      a0[2]= -0.5*(aX[2]-(1/(aX[2]*TimeM[1]))*(Psi_vMtrt[1,2]-Psi_vMctrl[1,2]) )
      a0[1]= -TimeM[1]*a0[2]
      ##update IM SM with the revised a0 values; earlier a0=c(0,0), thus just add a0 to IM SM 
      IM=a0[1]+aX[1]*X+aZ[1]*Z + vMtrt[ ,1]*X + vMctrl[ ,1]*(1-X)
      SM=a0[2]+aX[2]*X+aZ[2]*Z + vMtrt[ ,2]*X + vMctrl[ ,2]*(1-X)
      
      bZ=bIM=bSM=bX=bIMSM=rep(NA, 2)
      bIMSM[1]=sqrt( f2IMSM[1]*vIY/((0.15+1)*Psi_vMctrl[1,1]*Psi_vMctrl[2,2]))*0.75
      bIMSM[2]= sqrt( f2IMSM[2]*vSY/((0.15+1)*Psi_vMctrl[1,1]*Psi_vMctrl[2,2]))*1
      ...
      # TT=12: 4.8; TT=6: 5.3; TT=3: 5.3
      Psi_vY=diag(c(NA, 6),2,2)
      Psi_vY[1,1]=(TimeY[1]^2)*Psi_vY[2,2]
    }
    
    # f2IMSM=c(0.15,0 )
    if (f2IMSM[1]==0.15 & f2IMSM[2]==0){
      # try seting a0[1:2] to satisfy the randomization constraint on Y1; ## this same following term is multiplied to both bIY.IMSM and bSY.IMSM:  (\alpha_{IM0}\alpha_{SM.X}$+$\alpha_{SM0}\alpha_{IM.X} +  \alpha_{IM.X}\alpha_{SM.X}  + \psi_{v_{IM}v_{SM}}^{Trt}-\psi_{v_{IM}v_{SM}}^{Ctrl})  
      ##set aIM0= -aSM0*TimeM[1] ; parallel to  \alpha_{IM.X}+\alpha_{SM.X}Time_{M1}=0 
      a0[2]= -0.5*(aX[2]-(1/(aX[2]*TimeM[1]))*(Psi_vMtrt[1,2]-Psi_vMctrl[1,2]) )
      a0[1]= -TimeM[1]*a0[2]
      ...
      bZ=bIM=bSM=bX=bIMSM=rep(NA, 2)
      bIMSM[1]=sqrt( f2IMSM[1]*vIY/((0.15+1)*Psi_vMctrl[1,1]*Psi_vMctrl[2,2]))*1
      bIMSM[2]= sqrt( f2IMSM[2]*vSY/((0.15+1)*Psi_vMctrl[1,1]*Psi_vMctrl[2,2]))*0.8
  ...
  if( rho12_vM1==0.3 ){
    ...
    if (f2IMSM[1]==0.15 & f2IMSM[2]==0.15){
      bZ=bIM=bSM=bX=bIMSM=rep(NA, 2)
      bIMSM[1]=sqrt( f2IMSM[1]*vIY/((0.15+1)*Psi_vMctrl[1,1]*Psi_vMctrl[2,2]))*0.7
      bIMSM[2]= -bIMSM[1]/TimeY[1]
      bZ[1]=sqrt(0.02*vIY/((0.15+1)*1))*0.1
      # bZ[2]= sqrt(0.02*vSY/((0.15+1)*1))*0.8
      bZ[2]=-bZ[1]/TimeY[1]
      bIM[1]=sqrt(0.15*vIY/((0.15+1)*Psi_vMctrl[1,1]))*0.78 ##diag(Psi_vMctrl)=diag(Psi_vMTrt)
      # bIM[2]= sqrt(0.15*vSY/((0.15+1)*Psi_vMctrl[1,1]))*0.9
      bIM[2]=-bIM[1]/TimeY[1]
      bSM[1]=sqrt(0.15*vIY/((0.15+1)*Psi_vMctrl[2,2]))*0.78##diag(Psi_vMctrl)=diag(Psi_vMTrt)
      # bSM[2]= sqrt(0.15*vSY/((0.15+1)*Psi_vMctrl[2,2]))*0.9
      bSM[2]=-bSM[1]/TimeY[1]
      bX[1]=0.5*sqrt(vIY)*0.3
      bX[2]= -bX[1]/TimeY[1]
    ...
    # f2IMSM=c(0,0.15)
    if (f2IMSM[1]==0 & f2IMSM[2]==0.15){
      ##not work a0[1]= -(Psi_vMtrt[1,2]-Psi_vMctrl[1,2]+aX[1]*aX[2])/aX[2] #not work, bias X_SM, the calculation of which involves a0[1]
      # try seting a0[1:2] to satisfy the randomization constraint on Y1; ## this same following term is multiplied to both bIY.IMSM and bSY.IMSM:  (\alpha_{IM0}\alpha_{SM.X}$+$\alpha_{SM0}\alpha_{IM.X} +  \alpha_{IM.X}\alpha_{SM.X}  + \psi_{v_{IM}v_{SM}}^{Trt}-\psi_{v_{IM}v_{SM}}^{Ctrl})  
      ##set aIM0= -aSM0*TimeM[1] ; parallel to  \alpha_{IM.X}+\alpha_{SM.X}Time_{M1}=0 
      a0[2]= -0.5*(aX[2]-(1/(aX[2]*TimeM[1]))*(Psi_vMtrt[1,2]-Psi_vMctrl[1,2]) )
      a0[1]= -TimeM[1]*a0[2]
    ...
    if (f2IMSM[1]==0.15 & f2IMSM[2]==0){
      ...
      bZ=bIM=bSM=bX=bIMSM=rep(NA, 2)
      bIMSM[1]=sqrt( f2IMSM[1]*vIY/((0.15+1)*Psi_vMctrl[1,1]*Psi_vMctrl[2,2]))*1
      bIMSM[2]= sqrt( f2IMSM[2]*vSY/((0.15+1)*Psi_vMctrl[1,1]*Psi_vMctrl[2,2]))*0.78
